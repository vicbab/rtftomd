...
module Converter
    def convert(options)
        ...
        if status.success?
            puts "Converted #{@file} to #{@file}.md"
            FileUtils.cp("#{@file}.md", "#{@file}_copie.md")

            begin
                contents = File.read("#{@file}.md", encoding: "latin-1")

                contents = replace_odds(contents)

                File.open("#{@file}.md", 'w', encoding: "latin-1") do |file|
                    file.write(contents)
                end

                puts "Done replacing #{@file}.md"
            rescue => e
    ...
    module_function :convert, :replace_wrong_quotes, :replace_quotes, :replace_odds
end
